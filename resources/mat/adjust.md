...
| 항목                 | 값                    | 설명                    |
| ------------------ | -------------------- | --------------------- |
| toss\_click\_id    | `{TOSS_TK_CLICK_ID}` | 클릭 발생 시 부여되는 고유 클릭 ID |
| idfa / gps\_adid   | `{IDFA} / {AIFA}`    | 디바이스 광고 ID            |
| toss\_campaign\_id | `{TOSS_CID}`         | 캠페인 ID                |
...
| 항목                 | 값                    | 설명     |
| ------------------ | -------------------- | ------ |
| toss\_creative\_id | `{TOSS_CREATIVE_ID}` | 소재 ID  |
| toss\_site\_id     | `{SITE_ID}`          | 매체 ID  |
| campaign           | `{CAMPAIGN_NAME}`    | 캠페인명   |
| adgroup            | `{ADGROUP_NAME}`     | 광고 그룹명 |
