...
상품이 많은 판매처일수록 상품마다 광고를 따로 만들고 정보를 최신으로 유지하기가 어려워요. 카탈로그에 상품 목록을 한 번 등록해두면 이렇게 편해져요.
...
상품별 성과를 측정하려면 카탈로그의 **상품 ID**가 전환 이벤트의 `product_id`와 같아야 하고, **랜딩 URL**에서 전환이 추적되어야 해요. 전환 연동을 완료해야 이 두 값을 정확히 맞출 수 있어요. 자세한 입력 규칙은 카탈로그 연동 스펙을 참고해 주세요.
...
| 방식        | 이런 경우에 좋아요                                                          | 조건                                            |
| --------- | ------------------------------------------------------------------- | --------------------------------------------- |
| **자동 등록** | <p>• 상품 가격, 재고가 자주 바뀌는 경우<br>• 상품 목록 URL을 등록하면 주기적으로 최신 정보를 가져옴</p> | `https://`로 시작하는 URL, 파일 4GB 이하, 상품 500만 건 이하 |
| **수동 등록** | <p>• 상품 변동이 거의 없어 한 번만 등록하면 되는 경우<br>• CSV, TSV 파일로 직접 등록</p>       | 파일 100MB 이하, 상품 10만 건 이하                      |
...
1. 카탈로그 생성 화면에서 카탈로그명과 상품 판매처명을 입력하는 것까지는 자동 등록과 같아요. 상품 등록 방식에서 **수동 등록**을 선택해 주세요.
...
자동 등록이든 수동 등록이든 생성 버튼을 누르면 상품 등록이 시작돼요. 창을 닫아도 등록은 백그라운드에서 계속되고, 완료되면 이메일로 알려드려요. 상품 수가 많으면 등록에 시간이 걸릴 수 있어요.
...
{% hint style="info" %}
💡 **상품을 빼고 싶을 때는 삭제보다 미사용을 권장해요.** 자동 등록을 사용 중이라면, 상품을 삭제해도 다음 자동 업데이트 때 상품 목록 URL에 해당 상품 ID가 남아 있으면 다시 등록될 수 있어요. 미사용으로 설정하면 자동 업데이트와 관계없이 광고에 노출되지 않아요.
{% endhint %}
