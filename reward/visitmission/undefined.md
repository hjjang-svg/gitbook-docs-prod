---
hidden: true
layout:
  width: default
  title:
    visible: true
  description:
    visible: false
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: true
  metadata:
    visible: true
  tags:
    visible: true
  actions:
    visible: true
---

# 소재 가이드

### 소재 심사 제출

일주일 방문 미션(협력 광고)는 집행 당일 소재를 만들 수 없어요.

심사를 완료한 소재만 광고를 집행할 수 있어요.&#x20;

{% hint style="info" %}
원활한 광고 집행을 위해 집행일 기준 2\~3영업일 전에 소재를 제출해 주세요.

* **평일 10:00\~19:00**에 순차적으로 심사해요.
* 가이드에 맞지 않아 반려되는 경우에는 광고 집행 전까지 승인이 완료되지 않을 수 있어요.
* 광고 집행 전날 21:00 이후 심사가 끝나면 예산을 다 쓰지 못할 수 있어요.
{% endhint %}

**소재 수정**

* 소재는 **\[심사 전]** 상태에서 수정할 수 있어요.&#x20;
  * 집행일이 지나지 않은 캠페인은 \[반려] 상태에서도 수정할 수 있어요.
* \[심사 중], \[승인] 상태이거나 캠페인이 \[집행 예정], \[집행 중], \[집행 완료], \[집행 중단] 상태인 경우에는 수정할 수 없어요.
* 심사 승인 이후 소재 수정이 필요한 경우 집행 중단한 뒤 새로 제출해주세요. **단,광고 집행 전날 21:00 이후 심사가 끝나면 예산을 다 쓰지 못할 수 있어요.**

***

### **필요한 정보**&#x20;

<figure><img src="../../.gitbook/assets/일주일 방문 미션 (1).png" alt=""><figcaption></figcaption></figure>

| 광고 소재         | 상품 구경하기 | 상품 담기 | 상품 좋아요 누르기 |
| ------------- | ------- | ----- | ---------- |
| 1. 브랜드 로고 이미지 | ✓       | ✓     | ✓          |
| 2. 브랜드명       | ✓       | ✓     | ✓          |
| 3. 상품명        | ✓       | ✓     | ✓          |
| 4. 상품 이미지     | ✓       | ✓     | ✓          |
| 5. 랜딩 페이지 url | ✓       | ✓     | ✓          |

### 소재 만들기&#x20;

{% hint style="warning" %}
[**소재 심사 규정**](https://toss-ads.gitbook.io/guide/policy/creativeguide)**을 반드시 확인해주세요.**
{% endhint %}

<figure><img src="../../.gitbook/assets/올방미 소재 정보 (1).png" alt=""><figcaption></figcaption></figure>

1. **소재명**
   * 소재 이름을 입력하는 영역이에요. (100자 까지 입력할 수 있어요.)
   * 기본으로 년월일시로 자동 생성되고, 수정할 수 있어요.
2. **소재 유형**
   * 소재 유형마다 브릿지 페이지에서 보여주는 화면과 단가가 달라요.
   * 상품 구경하기 : 단가 20원
   * 상품 담기, 좋아요 누르기 : 단가 30원
3. **집행 기간**
   * 소재마다 캠페인과 동일한 집행기간 또는 별도로 직접 설정할 수 있어요.
   * 직접 설정하는 경우, 하루 단위로 최소 하루 \~ 최대 2주 선택할 수 있어요.
     * 과거 \~ 오늘은 선택할 수 없고, 다음날부터 선택할 수 있어요.
     * 캠페인 기간보다 이전, 이후의 날짜는 선택할 수 없어요.
4. **일 예산**
   * 5만원 이상 1억원 이하로 입력할 수 있어요.
   * 100원 단위로 입력해야 해요.
   * 일 예산을 입력하면 예상되는 목표 미션 완료 수를 보여줘요.

<figure><img src="../../.gitbook/assets/카버누 생성 - 소재 (2).png" alt=""><figcaption></figcaption></figure>

1. <mark style="background-color:$primary;">**카테고리 및 광고 고지 문구  <- 이미지 예시 수정 확인 후 라이브 필요**</mark>&#x20;
   * 화면 하단 ‘상품 구경하러 가기’ 버튼 위쪽에 표시돼요.
   * 카테고리에 따라 소재 제출 시 광고 고지 문구를 함께 제출해야 해요.
     * 자세한 내용은 [**업종별 심사 가이드**](https://toss-ads.gitbook.io/guide/policy/industryguide)에서 확인할 수 있어요.

<figure><img src="../../.gitbook/assets/image (6) (7).png" alt=""><figcaption></figcaption></figure>

2. **브랜드 로고 이미지**
   * 배경색이 포함된 400\*400px의 jpg,jpeg,png 형식의 이미지를 등록해주세요.
   * 로고만 삽입된 정사각형 이미지를 사용해주세요.
     * 제품 이미지 및 기획전 이미지는 사용할 수 없어요.
   * 투명 배경, 강한 형광색, 토스의 배경 색상과 같은 컬러의 배경 이미지는 사용할 수 없어요.
   * 테두리가 포함된 이미지는 사용할 수 없어요.
3. **브랜드명**
   * 공백 포함 최대 20자까지 입력할 수 있어요.
   * 브랜드명이 담긴 홍보 문구를 쓸 수 있어요.
     * 예시) 토스쇼핑 특가상품, 토스쇼핑 특별전
     * 홍보 문구는 랜딩페이지에서 확인 가능한 정보여야해요.
4. **상품명**
   * 공백 포함 최대 14자까지 입력할 수 있어요.
   * 랜딩 페이지와 상품명이 일치하지 않더라도 주요 키워드가 포함되어있으면 사용할 수 있어요.
5.  **상품 이미지**

    * 배경색이 포함된 1000\*1000px의 jpg, jpeg, png 형식의 이미지를 등록해주세요.
    * 상품의 특징이 한눈에 잘 보이는 이미지로 등록해주세요.
    * 모서리가 둥글지 않은 정사각형 이미지로 등록해주세요.
    * 투명 배경, 강한 형광색, 토스의 배경 색상과 같은 컬러의 배경 이미지는 사용할 수 없어요.
    * 테두리가 있는 이미지는 사용할 수 없어요.
    * 주요 제품을 강조하기 위해 최대한 가운데에 배치하여 구성해주세요.
    * 컬러 테두리가 있거나 작게 분할되어 3개 이상 복잡한 오브젝트로 구성된 이미지는 사용할 수 없어요.
      * 가독성을 위해 엠블럼은 최대 2개까지만 사용할 수 있어요.

    ※ 이미지 내 텍스트는 가독성을 위해 사용을 지양해주세요.

    * 부득이 사용이 필요하다면 배경, 오브젝트 또는 다른 텍스트에 가려지지 않도록 사용해주세요.
6. **랜딩 페이지 url**
   * 올리브영 상품 상세 페이지 혹은 라이브 사전 예고 페이지만 등록할 수 있어요.
     * **상품 상세페이지**
       * utm은 자동으로 생성되기 때문에 상품 상세페이지 url만 넣어주세요.
     * **라이브 사전 예고 페이지**
       * 라이브 당일 예고 페이지 url은 소재로 등록할 수 없어요.
         * 예시) 26년 8월 1일에 라이브 방송을 진행하는 상품이라면 26년 7월 31일까지만 소재로 등록할 수 있어요.

> **링크 작성 가이드**

* utm은 자동으로 생성되기 때문에 아래 형식으로만 제출해주세요.
  * 링크가 가이드 형식과 다르면 광고를 집행할 수 없어요.
    * **올리브영 상품 상세 페이지**
      * `https://m.oliveyoung.co.kr/m/goods/getGoodsDetail.do?goodsNo=상품번호`
    * **라이브 사전 예고 페이지**
      * `https://m.oliveyoung.co.kr/m/mtn/liveshop/teasers/예고ID`

> **랜딩 페이지 심사 규정**

* 올리브영 상품 상세 페이지로만 집행 가능해요.
* 광고 소재 내용과 랜딩 페이지 내용이 **서로 다르면** 광고를 집행할 수 없어요.
* 랜딩 페이지에서 정보를 바로 확인할 수 있어야 해요.
* 랜딩 페이지에 유저가 상품·이벤트·프로모션을 문의할 수 있는 문의처가 적혀 있어야 해요.
* 공식 제휴 광고가 아니라면 '토스'의 브랜드명·서비스명은 사용할 수 없어요.
  * 내부 협의가 이루어진 경우, 증빙 자료를 제출해주세요.
* 저작권 침해 등 이슈가 생길 수 있는 소재는 집행할 수 없어요.

