...
* **카탈로그 연동(토스 네이티브 광고):** 여러 상품을 담은 카탈로그를 연동하면, 관심사에 맞는 고객에게 상품이 카드형 광고로 자동 노출돼요. 상품 수가 많고, 고객마다 맞는 상품을 자동으로 보여주고 싶을 때 적합해요.
...
### 에이전틱 커머스 <a href="#agentic-commerce" id="agentic-commerce"></a>

에이전틱 커머스를 사용하면 고객이 주문서를 더 간편하게 작성할 수 있어요.

고객이 동의하면 토스에 저장된 이름·연락처·배송지 등을 주문서에 자동으로 입력해요.

정보 입력 부담을 덜어 구매 과정의 이탈을 줄이는 데 도움이 돼요. 자동 입력 중에도 고객은 언제든 멈추고 직접 입력할 수 있어요.

<figure><img src="../../.gitbook/assets/order-info-auto-fill-guide-flow-original.png" alt="주문 정보 자동 입력 중, 주문 정보 확인, 필수 항목 동의 상태를 각각 보여주는 화면"><figcaption></figcaption></figure>

#### 사용하기

기능을 사용하려면 먼저 [상품 카탈로그를 등록](../../catalog/how-to.md)해 주세요. 에이전틱 커머스는 광고계정마다 설정할 수 있고, 기본으로 꺼져 있어요.

<figure><img src="../../.gitbook/assets/에이전틱 커머스 - 1.png" alt="광고 관리 화면의 도구 탭에서 에이전틱 커머스 토글을 켜는 화면"><figcaption></figcaption></figure>

1. 광고 관리 화면의 **도구** 탭에서 **에이전틱 커머스 토글**을 켜 주세요.
2. 기능 안내를 읽고 사용에 동의해 주세요.
3. 토스가 연결된 쇼핑몰의 구매 환경을 확인해요. 확인에 영업일 기준 최대 2일 정도 소요될 수 있어요.
4. 확인이 끝나면 화면에서 연결 결과를 확인해 주세요.

<figure><img src="../../.gitbook/assets/에이전틱 커머스 - 2 (1).png" alt=""><figcaption></figcaption></figure>

* **연결 성공:** **작동 중**으로 표시돼요. 에이전틱 커머스를 사용할 수 있는 상태예요.
* **연결 실패:** **사용 불가**로 표시돼요.

#### 사용 불가일 때 확인하기 <a href="#undefined-1" id="undefined-1"></a>

에이전틱 커머스를 사용하려면 쇼핑몰이 다음 조건을 모두 충족해야 해요.

* 웹페이지에서 상품을 구매할 수 있어요.
* 비회원으로 상품을 구매할 수 있어요.

하나라도 충족하지 않으면 **사용 불가**로 표시돼요. 쇼핑몰 설정을 변경한 뒤 다시 확인해 주세요.

두 조건을 모두 충족해도 구매 환경에 따라 **사용 불가**로 표시될 수 있어요. 문제가 해결되지 않으면 [토스애즈 고객센터](http://toss-business.channel.io/)로 문의해 주세요.

#### 사용 중지하기

광고 관리 화면의 **도구** 탭에서 **에이전틱 커머스 토글**을 꺼 주세요.
