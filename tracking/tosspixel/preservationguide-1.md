...
A. 서비스에서 겹치지 않는 값을 직접 만들어 넣어주세요. 값을 만들기 어렵다면 event\_id 없이 보내도 이벤트는 정상적으로 수집돼요.

**Q. order\_id가 있는데 event\_id를 또 보내야 하나요?**

A. `order_id`는 리포트에서 주문을 확인하기 위한 파라미터이고, event\_id는 이벤트 자체를 식별하는 값이라 역할이 달라요. 구매 이벤트라면 두 값에 같은 주문번호를 넣어도 괜찮아요.
