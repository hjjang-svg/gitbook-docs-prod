...
* **토스 픽셀**은 [전환 추적 코드 생성하기 가이드](https://toss-ads.gitbook.io/guide/tracking/tag/code_generation)에 따라 **전환 추적 코드를 생성한 후에 사용**할 수 있어요. 발급받은 전환 추적 코드는 반드시 따옴표(`' '` 또는 `" "`)로 감싸서 스크립트에 입력해 주세요.&#x20;
* 예시의 `'전환 코드'` 부분을 발급받은 실제 값으로 바꾸고, 따옴표는 그대로 유지해 주세요.
{% endhint %}
...
{% hint style="info" %}
* 서로 다른 전환 이벤트에 동일한 `event_id`를 반복해서 사용하면 중복 이벤트로 처리되어 전환 수집 또는 집계에 이상이 발생할 수 있어요. `event_id`는 전환 이벤트마다 고유한 값을 입력할 수 있을 때만 사용해 주세요.&#x20;
* 고유한 값을 지정하기 어렵다면 `event_id`를 생략해 주세요.
{% endhint %}
...
*   표준 이벤트에 해당하지 않는 전환을 추적할 때 사용해요.
    ...
    이벤트명은 직접 지정할 수 있고, 표준 이벤트와 동일한 파라미터를 사용할 수 있어요.
